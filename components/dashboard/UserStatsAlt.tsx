...
const UserStatsAlt = ({ stats, theme: propTheme, elegantMode = true }: UserStatsProps) => {
  ...
  const renderStars = (value: number) => {
    ...
    const normalizedValue = Math.min(5, Math.ceil((value / maxStat) * 5));
    ...
    return (
      <div className="flex items-center space-x-1">
        {Array.from({ length: 5 }).map((_, index) => (
          <span 
            key={index}
            className={`text-xs ${index < normalizedValue ? 'text-white' : 'text-white/20'}`}
          >
            ★
          </span>
        ))}
    ...
    <div className="space-y-4">
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <div className="text-3xl font-bold text-white">{totalValue * 10}</div>
        </div>
          ...
          return (
            ...
            >
              ...
              <div className="flex justify-center">
                {renderStars(statKey === 'health' ? value / 10 : value)}
              </div>
            </motion.div>
          );
        })}
